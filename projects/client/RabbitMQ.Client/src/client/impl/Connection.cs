...
entry.ToString());
    ...
    public class AsyncConnection : IAsyncConnection
    {
        ...
        ///<summary>Heartbeat frame for transmission. Reusable across connections.</summary>
        public readonly AsyncFrame m_heartbeatFrame = new AsyncFrame(Constants.FrameHeartbeat, 0, new byte[0]);
        ...
        public EventHandler<EventArgs> m_connectionUnblocked;
        public IConnectionFactory m_factory;
        public IAsyncFrameHandler m_frameHandler;
        ...
        private AsyncTimer _heartbeatWriteTimer;
        private Timer _heartbeatReadTimer;
        public AutoResetEvent m_heartbeatRead = new AutoResetEvent(false);
        public AutoResetEvent m_heartbeatWrite = new AutoResetEvent(false);
        ...
        public AsyncConnection(IConnectionFactory factory, bool insist, IAsyncFrameHandler frameHandler)
        {
            KnownHosts = null;
            FrameMax = 0;
            m_factory = factory;
...
#pragma warning restore 0168
        ...
        public async Task FinishClose()
        {
            ...
            m_heartbeatWrite.Set();
            m_closed = true;
            await MaybeStopHeartbeatTimers().ConfigureAwait(false);

            await m_frameHandler.Close().ConfigureAwait(false);
            m_model0.SetCloseReason(m_closeReason);
            m_model0.FinishClose();
        }
...
entry.ToString());
        ...
        public void MaybeStartHeartbeatTimers()
        {
            if (Heartbeat != 0)
            {
                _heartbeatWriteTimer = new AsyncTimer(HeartbeatWriteTimerCallback);
                _heartbeatReadTimer = new Timer(HeartbeatReadTimerCallback);

                _heartbeatWriteTimer.Start(TimeSpan.FromMilliseconds(200), m_heartbeatTimeSpan);
#if NETFX_CORE        
                _heartbeatReadTimer.Change(200, m_heartbeatTimeSpan.Milliseconds);
#else
        ...
        public void HeartbeatReadTimerCallback(object state)
        {
            ...
            try
            {
                ...
                if (shouldTerminate)
                {
                    TerminateMainloop().GetAwaiter().GetResult();
                    FinishClose().GetAwaiter().GetResult();
                }
                else
                {
                    _heartbeatReadTimer.Change(Heartbeat * 1000, Timeout.Infinite);
        ...
        public async Task HeartbeatWriteTimerCallback()
        {
            bool shouldTerminate = false;
            try
            {
                try
                {
                    if (!m_closed)
                    {
                        await WriteFrame(m_heartbeatFrame).ConfigureAwait(false);
                        await m_frameHandler.Flush().ConfigureAwait(false);
                    }
                }
                catch (Exception e)
                {
                ...
                if (m_closed || shouldTerminate)
                {
                    // Daniel: Not sure about this
                    TerminateMainloop().Ignore(); // Fire & Forget or we would lock forever
                    FinishClose().Ignore(); // Fire & Forget or we would lock forever
                }
            }
            catch (ObjectDisposedException ignored)
            {
        ...
        protected Task MaybeStopHeartbeatTimers()
        {
            MaybeDisposeTimer(_heartbeatReadTimer);
            return _heartbeatWriteTimer.Stop();
        }
        ...
        /// May be called more than once. Should therefore be idempotent.
        ///</remarks>
        public async Task TerminateMainloop()
        {
            await MaybeStopHeartbeatTimers().ConfigureAwait(false);
            m_running = false;
        }
        ...
        public async Task WriteFrame(AsyncFrame f)
        {
            await m_frameHandler.WriteFrame(f).ConfigureAwait(false);
            m_heartbeatWrite.Set();
        }

        public async Task WriteFrameSet(IList<AsyncFrame> f)
        {
            await m_frameHandler.WriteFrameSet(f).ConfigureAwait(false);
            m_heartbeatWrite.Set();
        }
