...
**The rare exits were covered before the common one was.** Measured by counting all four exits of `HandleBasicDeliverAsync`: 3000 messages per channel, no prefetch limit, a 5 ms consumer, six ordinary `CloseAsync`/`DisposeAsync` rounds gave 4479 delivered, **2971 dropped by the `_disposed`/`IsQuiescing` guard**, and **zero** for the entry `ThrowIfCancellationRequested` and for *both* of the catch clauses #1988 added. The guard case is not a race at all: `Channel.CloseAsync` calls `Quiesce()` before transmitting `channel.close`, so every delivery arriving between that point and `close-ok` takes it. Treat the exact split as one configuration rather than a rate; what did not vary is that the guard fires in the hundreds per close and the catches do not fire.

**#2039 replaced per-exit disposal with one ownership rule**, because four exits each needing their own `Dispose` is the shape that produced the bug. `HandleBasicDeliverAsync` now sets a flag once the work channel has accepted the item and returns the body in a `finally` when it has not, which covers the entry throw, the guard, and a failed write uniformly. Two properties it rests on, both measured rather than assumed: an unbounded `WriteAsync` either accepts the item or throws and never does both, so the flag cannot lie; and the `finally` cannot return a body the reader also owns, because `work` is declared inside the `if` and so lies outside the `finally`'s scope - the compiler keeps the work item's copy out of reach rather than a convention doing it. The `ChannelClosedException` catch stays, with an empty body, for the reason below; the `OperationCanceledException` one is gone.

**Testing pool return has a thread-affinity trap, on net8.0 only.** The three drop-path tests assert that a distinctive array comes back out of `ArrayPool<byte>.Shared`, which works because the `finally` returns it on the caller's thread. A *success*-path return is made on the reader's worker thread, and net8.0's `TlsOverPerCoreLockedStacksArrayPool` caches it in that thread's local slot, so an observing thread's rents do not see it - measured: one return on thread 5, invisible to four consecutive rents on thread 20, and still invisible after that thread died and a GC ran. Two limits on that, both measured: it is **conditional**, because a second same-bucket return by the returning thread evicts the first into the per-core stacks where it becomes visible; and it is **framework-specific**, because net472 resolves `System.Buffers` 4.5.1, whose `DefaultArrayPool<T>` is a shared per-bucket stack with no `ThreadStatic` at all. So instance identity is unreliably-invisible on net8.0 and has no thread protection on net472.

**A success-path double return is catchable without touching production code, and the claim that it needed an injected pool was wrong.** Body integrity catches it directly, because the array goes back to the pool while a consumer is still reading it: publishing self-describing bodies and verifying them in the callback gave 0 corrupt bodies on a correct build and 156 under a mutation that disposed the work item on the success path. The existing `TestBasicPublish.TestBasicRoundtrip*` tests already kill that mutation in under a second, and a wider Integration run under it fails 42+ tests including a broker-side `FRAME_ERROR ... frame_too_large`, because the double return corrupts *outbound* frame buffers too - the reader returns on the same thread that rents them.

**Why a cancelled token used to need its own catch.** Measured against `System.Threading.Channels` on an unbounded channel: an already-cancelled token yields `TaskCanceledException`, not `ChannelClosedException`; and when the channel is completed *and* the token is cancelled, cancellation wins, so the `ChannelClosedException` handler does not run. That is a narrow corner rather than the ordinary teardown case - the entry throw has already returned, so it needs cancellation inside the few await-free instructions before the write, and once `_mainLoopCts` is cancelled the receive loop stops dispatching frames. **Which is why the delivery path no longer catches it.** #2039 removed that catch in favour of one `finally` on the body, so cancellation from either side - the entry throw or the write - returns the body and propagates unchanged. `ChannelClosedException` is still caught, and only to swallow. `TryWrite`, used by `ShutdownConsumer`, does not throw; it returns `false`.

**`WorkStruct.Dispose()` is not idempotent.** It is a readonly struct whose `RentedMemory` field is readonly, while `RentedMemory.Dispose()` is not declared readonly and mutates, so C# invokes it on a defensive copy and the `RentedArray = Array.Empty<byte>()` write-back is discarded. Confirmed on a minimal struct of the same shape: after one `Dispose` the field still referenced the original array, and after a second, two consecutive `ArrayPool<byte>.Shared.Rent` calls returned the same instance. One array, two owners - worse than the leak. Every caller must dispose a given work item exactly once; today the reader loop disposes what it drains, and the delivery drop site disposes only the parameter, from a `finally` that cannot see the work item because it is scoped inside the `if`. A bounded channel, a retry around the write, or a drain running alongside a drop path would each have to re-establish that.
