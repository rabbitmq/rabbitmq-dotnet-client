...
namespace RabbitMQ.Client.Content
{
    ...
    /// Framework for constructing various types of AMQP. Basic-class application messages.
    /// </summary>
    public sealed class AsyncBasicMessageBuilder : IAsyncMessageBuilder
    {
        /// <summary>
        /// By default, new instances of BasicMessageBuilder and its subclasses will have this much initial buffer space.
        /// </summary>
        ...
        private MemoryStream m_accumulator;
        private AsyncNetworkBinaryWriter m_writer;
        ...
        /// The <see cref="DefaultAccumulatorSize"/> is used for the initial accumulator buffer size.
        /// </remarks>
        public AsyncBasicMessageBuilder(IModel model) : this(model, DefaultAccumulatorSize)
        {
        }
        ...
        /// Construct an instance ready for writing.
        /// </summary>
        public AsyncBasicMessageBuilder(IModel model, int initialAccumulatorSize)
        {
            Properties = model.CreateBasicProperties();
            m_accumulator = new MemoryStream(initialAccumulatorSize);
        ...
        /// instance is not reset.
        /// </remarks>
        public AsyncNetworkBinaryWriter Writer
        {
            get
            {
                if (m_writer == null)
                {
                    m_writer = new AsyncNetworkBinaryWriter(m_accumulator);
                }
                return m_writer;
            }
        }
        ...
        /// or null if none is available or relevant.
        /// </summary>
        public IContentHeader GetContentHeader()
        {
            return Properties;
        }
        ...
        /// Write a single byte into the message body, without encoding or interpretation.
        /// </summary>
        public async Task<IAsyncMessageBuilder> RawWrite(byte value)
        {
            await BodyStream.WriteAsync(new byte[] { value }, 0, 1).ConfigureAwait(false);
            return this;
        }
        ...
        /// Write a byte array into the message body, without encoding or interpretation.
        /// </summary>
        public Task<IAsyncMessageBuilder> RawWrite(byte[] bytes)
        {
            return RawWrite(bytes, 0, bytes.Length);
        }
        ...
        /// Write a section of a byte array into the message body, without encoding or interpretation.
        /// </summary>
        public async Task<IAsyncMessageBuilder> RawWrite(byte[] bytes, int offset, int length)
        {
            await BodyStream.WriteAsync(bytes, offset, length).ConfigureAwait(false);
            return this;
        }
    }
}
