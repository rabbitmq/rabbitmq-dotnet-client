...
namespace RabbitMQ.Util
{
   ...
   */
    ...
    public class UShortAllocator
    {
        private readonly ushort[] unsorted;
        private IntervalList Base;
        private ushort unsortedCount = 0;
     ...
     */
        ...
        public UShortAllocator(ushort start, ushort end)
        {
            if (start > end)
            {
        ...
        /**
     * Allocate a fresh integer from the range, or return null if no more values
     * are available. This operation is guaranteed to run in O(1)
     */

        public Nullable<ushort> Allocate()
        {
            if (unsortedCount > 0)
            {
        ...
        public bool Reserve(ushort id)
        {
            ...
            while (current != null)
            {
                ...
                else
                {
                    ...
                    ushort start = id;
                    ushort end = current.End;
                    start++;
                    end--;
                    var rest = new IntervalList(start, current.End);
                    current.End = end;
